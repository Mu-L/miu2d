...
Miu2D 是一个 **160,000 行**的 2D ARPG 引擎，使用 TypeScript 和 Rust 编写，通过**原生 WebGL** 渲染——不依赖 Unity、Godot、Phaser、PixiJS 或任何其他游戏框架。所有子系统——精灵批量渲染、A* 寻路、二进制格式解码、脚本虚拟机、天气粒子、屏幕特效——全部从零实现。
...
- **可学习的干净架构** — 7 层类继承体系（Sprite → CharacterBase → Movement → Combat → Character → PlayerBase → PlayerCombat → Player）职责分明，是学习完整 2D ARPG 引擎底层原理的理想参考
...
| 层级 | 包 | 行数 | 说明 |
|---|---|---:|---|
| **UI** | `@miu2d/game` | 31,174 | React 19 · 3 套主题（经典 / 现代 / 移动端）· 56 个组件 |
| **引擎** | `@miu2d/engine` | 59,342 | 纯 TypeScript · 213 个文件 · 19 个模块 · 不依赖 React |
| ↳ 渲染器 | `renderer/` | 2,838 | 原生 WebGL · SpriteBatcher · Canvas2D 回退 · GLSL 滤镜 |
| ↳ 脚本 VM | `script/` | 5,879 | 182 条指令 · 自研解析器 + 异步执行器 |
| ↳ 角色系统 | `character/` | 6,415 | 7 层继承链 · NPC AI · 贝塞尔曲线移动 |
| ↳ 武功系统 | `magic/` | 8,702 | 22 种 MoveKind 轨迹 · 10 种 SpecialKind 状态效果 |
| **WASM** | `@miu2d/engine-wasm` | 2,644 | Rust → WebAssembly · A\* 寻路 · 解码器 · 空间哈希 · zstd |
| **后端** | `@miu2d/server` | 13,700 | Hono + tRPC + Drizzle ORM · 22 张 PostgreSQL 表 · 17 个路由 |
| **编辑器** | `@miu2d/dashboard` | 34,731 | VS Code 风格布局 · 13 个编辑模块 |
