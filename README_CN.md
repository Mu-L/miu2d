...
绝大多数 Web 游戏项目会选择 PixiJS、Phaser，或编译 Unity/Godot 到 WASM。Miu2D 走了一条不同的路：整个渲染管线直接与 `WebGLRenderingContext` 对话，寻路器用 Rust 编译到 WASM 并通过零拷贝共享内存传输数据，脚本引擎通过自研解析器/执行器解释 182 条游戏指令。最终得到的是一个每一层都可见、可调试、专为 2D ARPG 机制定制的系统。
...
| 层级 | 包 | 行数 | 说明 |
|---|---|---:|---|
| **UI** | `@miu2d/game` | 31,615 | React 19 · 3 套主题（经典 / 现代 / 移动端）· 84 个组件 |
| **引擎** | `@miu2d/engine` | 64,650 | 纯 TypeScript · 215 个文件 · 19 个模块 · 不依赖 React |
| ↳ 渲染器 | `renderer/` | 3,135 | 原生 WebGL · SpriteBatcher · Canvas2D 回退 · GLSL 滤镜 |
| ↳ 脚本 VM | `script/` | 6,205 | 218 条指令 · 自研解析器 + 异步执行器 |
| ↳ 角色系统 | `character/` | 6,410 | 8 层继承链 · NPC AI · 贝塞尔曲线移动 |
| ↳ 武功系统 | `magic/` | 8,798 | 22 种 MoveKind 轨迹 · 10 种 SpecialKind 状态效果 |
| **WASM** | `@miu2d/engine-wasm` | 3,160 | Rust → WebAssembly · A\* 寻路 · 解码器 · 空间哈希 · zstd |
| **后端** | `@miu2d/server` | 14,143 | Hono + tRPC + Drizzle ORM · 21 张 PostgreSQL 表 · 19 个路由 |
| **编辑器** | `@miu2d/dashboard` | 35,150 | VS Code 风格布局 · 13 个编辑模块 |
...
| **脚本 / 事件系统** | `script/` | 自研虚拟机：解析器 + 异步执行器，218 条指令涵盖 9 大类（对话、玩家、NPC、状态、音频、特效、物体、物品、杂项） |
...
```
Sprite (656 行)
 └─ CharacterBase (963) — 属性、数值、状态标记
     └─ CharacterMovement (1,051) — A* 寻路、格子行走、贝塞尔曲线
         └─ CharacterCombat (784) — 攻击、伤害计算、状态效果
             └─ Character (987) — NPC/玩家共享逻辑 [抽象类]
                 ├─ PlayerBase → PlayerCombat → Player (合计 2,696 行)
                 └─ Npc (659) — AI 行为、交互脚本、空间网格
```
...
项目内置 **35,150 行**的 VS Code 风格游戏编辑器，包含活动栏、侧边栏和内容面板：
...
| 包名 | 代码量 | 职责 |
|------|------:|------|
| `@miu2d/engine` | 64,650 | 纯 TS 游戏引擎 — 19 模块，不依赖 React |
| `@miu2d/dashboard` | 35,150 | VS Code 风格游戏数据编辑器（13 模块） |
| `@miu2d/game` | 31,615 | 游戏运行时 + 3 套 UI 主题（经典/现代/移动端） |
| `@miu2d/server` | 14,143 | Hono + tRPC 后端（21 表、19 路由） |
| `@miu2d/types` | 12,183 | 共享 Zod 4 Schema（18 个领域模块） |
| `@miu2d/web` | 4,973 | 应用壳、路由、落地页 |
| `@miu2d/converter` | 4,354 | Rust CLI：ASF/MPC → MSF、MAP → MMF 批量转换 |
| `@miu2d/engine-wasm` | 3,160 | Rust → WASM：寻路、解码器、空间哈希、zstd |
| `@miu2d/viewer` | 3,151 | 资源查看器（ASF/地图/MPC/音频） |
| `@miu2d/ui` | 1,775 | 通用 UI 组件（无业务依赖） |
| `@miu2d/shared` | 990 | i18n、tRPC 客户端、React Context |

### 引擎模块细分

| 模块 | 代码量 | 职责 |
|------|------:|------|
| `magic/` | 8,798 | 22 种 MoveKind 轨迹、效果、被动、精灵工厂 |
| `character/` | 6,410 | 8 层继承链、属性、战斗、移动 |
| `runtime/` | 6,252 | GameEngine、GameManager、InputHandler、CameraController |
| `script/` | 6,205 | 218 条指令脚本 VM（解析器 + 执行器 + 9 类指令） |
| `player/` | 5,839 | 玩家控制器、背包、武功槽、装备 |
| `gui/` | 4,560 | GUI 管理器、对话系统、购买界面、UI 桥接 |
| `npc/` | 3,843 | NPC AI、交互脚本、空间网格、武功缓存 |
| `renderer/` | 3,135 | WebGL + Canvas2D 渲染器、精灵/矩形批处理、GLSL 着色器 |
| `resource/` | 2,940 | 资源加载器、8 种二进制格式解码器 |
| `storage/` | 2,132 | 存档/读档系统、游戏状态持久化 |
| `obj/` | 1,978 | 场景物体（宝箱、门、陷阱）、管理器 + 渲染器 |
| `map/` | 1,638 | 地图解析、障碍网格、瓦片渲染、陷阱区域 |
| `weather/` | 1,533 | 雨、雪、屏幕水滴、闪电 |
| `wasm/` | 1,202 | WASM 桥接层（寻路、解码器、碰撞） |
| `core/` | 1,109 | 引擎上下文、类型、日志器、游戏 API |
| `utils/` | 989 | 方向、距离、碰撞、INI 解析器 |
| `sprite/` | 903 | 精灵基类、边缘检测 |
| `audio/` | 782 | Web Audio API 管理器（OGG/MP3/WAV） |
| `data/` | 485 | 数据模型与配置定义 |
